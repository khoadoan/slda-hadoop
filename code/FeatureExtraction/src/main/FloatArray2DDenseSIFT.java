 ...
 * @version 0.1b
 */
public class FloatArray2DDenseSIFT extends
    FloatArray2DFeatureTransform<FloatArray2DDenseSIFT.Param> {
  final static public class Param implements Serializable {
    ...
    public boolean equals(final Param p) {
      return (fdSize == p.fdSize) && (fdBins == p.fdBins) && (maxOctaveSize == p.maxOctaveSize)
          && (minOctaveSize == p.minOctaveSize) && (steps == p.steps)
   ...
   * @param feature_descriptor_size
   */
  public FloatArray2DDenseSIFT(final Param p) {
    super(p);
    octaves = null;
   ...
   * 
   * @param src image having a generating gaussian kernel of {@link Param#initialSigma} img must be
   *        a 2d-array of float values in range [0.0f, ..., 1.0f]
   */
  @Override
  final public void init(FloatArray2D src) {
    ...
    while (w > Math.max(max_kernel_size, p.minOctaveSize - 1)
        && h > Math.max(max_kernel_size, p.minOctaveSize - 1)) {
      w /= 2.0f;
      h /= 2.0f;
      ++o;
    }
   ...
   * @param octave_sigma sigma of the corresponding gaussian kernel with respect to the scale octave
   * @param orientation orientation [-&pi; ... &pi;]
   */
  private float[] createDescriptor(final float[] c, final int o, final float octave_sigma,
    ...
    region[0] = new FloatArray2D(fdWidth, fdWidth);
    region[1] = new FloatArray2D(fdWidth, fdWidth);
    ...
    for (int y = fdWidth - 1; y >= 0; --y) {
      final float ys = ((float) y - 2.0f * (float) p.fdSize + 0.5f) * octave_sigma; // !< scale y
                                                                                    // around 0,0
      for (int x = fdWidth - 1; x >= 0; --x) {
        final float xs = ((float) x - 2.0f * (float) p.fdSize + 0.5f) * octave_sigma; // !< scale x
                                                                                      // around 0,0
        ...
        final int yg = Util.pingPong((int) (Math.round(yr + c[1])), gradients[0].height);

        // translate xs to sample x position in the gradient image
        final int xg = Util.pingPong((int) (Math.round(xr + c[0])), gradients[0].width);
      ...
      for (int x = p.fdSize - 1; x >= 0; --x) {
        ...
        for (int ysr = 3; ysr >= 0; --ysr) {
          ...
          for (int xsr = 3; xsr >= 0; --xsr) {
            final float bin_location = (region[1].data[yp + xp + ysrp + xsr] + (float) Math.PI)
                / (float) fdBinWidth;
    ...
    for (int y = p.fdSize - 1; y >= 0; --y) {
      for (int x = p.fdSize - 1; x >= 0; --x) {
        for (int b = p.fdBins - 1; b >= 0; --b) {
          desc[i] = hist[y][x][b];
          max_bin_val += (double) desc[i] * desc[i]; // L2 normalization
  ...
  public void extractDenseFeatures(int stepSize, int width, int height,
      final Collection<Feature> features) {

    // these two statements follow the original implementation
    final FloatArray2DScaleOctave octave = octaves[0];
    octave.build();
    final float octave_sigma = octave.SIGMA[0];

    float orientation = 0;

    // first, generate the points based on the step size
    // the size of the block is 4 * scale
...
//    float halfSize = (float) Math.ceil(fdWidth / 2);
    ...
    for (int x = (int) halfSize; x < width; x = x + stepSize) {
      ...
      for (int y = (int) halfSize; y < height; y = y + stepSize) {
        ...
        features.add(new Feature(octave_sigma * 1 /* 2^0 */, orientation, new float[] { x, y },
            createDescriptor(cParam, 0, octave_sigma, orientation)));
   ...
   */
  final protected void processCandidate(final float[] c, final int o, final List<Feature> features) {
    ...
    final float octave_sigma = octave.SIGMA[0]
        * (float) Math.pow(2.0f, c[2] / (float) octave.STEPS);

    // create a circular gaussian window with sigma 1.5 times that of the feature
    final FloatArray2D gaussianMask = Filter.createGaussianKernelOffset(octave_sigma * 1.5f, c[0]
        - (float) Math.floor(c[0]), c[1] - (float) Math.floor(c[1]), false);
    ...
    for (int yi = gaussianMask.width - 1; yi >= 0; --yi) {
      final int ra_y = src[0].width
          * Math.max(0, Math.min(src[0].height - 1, (int) c[1] + yi - half_size));
    ...
    // TODO this is for test
    // ---------------------------------------------------------------------
    // ImageArrayConverter.FloatArrayToImagePlus( gradientROI[ 0 ], "gaussianMaskedGradientROI", 0,
    // 0 ).show();
    // ImageArrayConverter.FloatArrayToImagePlus( gradientROI[ 1 ], "gaussianMaskedGradientROI", 0,
    // 0 ).show();

    // build an orientation histogram of the region
    for (int i = 0; i < gradientROI[0].data.length; ++i) {
      final int bin = Math
          .max(0, (int) ((gradientROI[1].data[i] + Math.PI) / ORIENTATION_BIN_SIZE));
    ...
    // assign descriptor and add the Feature instance to the collection
    features.add(new Feature(octave_sigma * scale, orientation, new float[] { c[0] * scale,
        c[1] * scale },
    // new float[]{ ( c[ 0 ] + 0.5f ) * scale - 0.5f, ( c[ 1 ] + 0.5f ) * scale - 0.5f },
        createDescriptor(c, o, octave_sigma, orientation)));

    /**
     * check if there is another significant orientation ( > 80% max ) if there is one, duplicate
     * the feature and
     */
    for (int i = 0; i < ORIENTATION_BINS; ++i) {
      if (i != max_i && (max_i + 1) % ORIENTATION_BINS != i
        ...
        if (e0 < e1 && e2 < e1) {
          ...
          features.add(new Feature(octave_sigma * scale, orientation, new float[] { c[0] * scale,
              c[1] * scale }, createDescriptor(c, o, octave_sigma, orientation)));
        }
      }
    }
    return;
   ...
   * @return matches
   */
  public static Vector<PointMatch> createMatches(final List<Feature> fs1, final List<Feature> fs2,
      final float rod) {
    ...
    for (final Feature f1 : fs1) {
      ...
      if (best != null && second_best_d < Float.MAX_VALUE && best_d / second_best_d < rod)
        matches.addElement(new PointMatch(
            new Point(new float[] { f1.location[0], f1.location[1] }), new Point(new float[] {
                best.location[0], best.location[1] }), (f1.scale + best.scale) / 2.0f));
    }
   ...
   */
  public static Vector<PointMatch> createMatches(final List<Feature> fs1, final List<Feature> fs2,
      final float max_sd, final AbstractModel<?> model, final float max_id, final float rod) {
    final Vector<PointMatch> matches = new Vector<PointMatch>();
    ...
    for (final Feature f1 : fs1) {
      ...
      if (max_sd < Float.MAX_VALUE) {
        ...
        // System.out.println( "first = " + first + ", last = " + last + ", first.scale = " +
        // fs2.get( first ).scale + ", last.scale = " + fs2.get( last ).scale + ", this.scale = " +
        // f1.scale );
      }
   ...
   * @param rs
   */
  public static float[] featureSizeHistogram(final Vector<Feature> features, final float min,
      final float max, final int bins) {
    ...
    for (final Feature f : features) {
      final int bin = (int) Math.max(0,
          Math.min(bins - 1, (int) (Math.log(f.scale) / Math.log(2.0) * 28.0f)));
      ++hb[bin];
    }
  ...
  final public float getInitialSigma() {
    return p.initialSigma;
  }
  ...
  final public int getMaxOctaveSize() {
    return p.maxOctaveSize;
  }
}
