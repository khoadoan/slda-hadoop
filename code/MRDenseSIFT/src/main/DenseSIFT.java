 ...
 * @version 0.4b
 */
public class DenseSIFT extends FeatureTransform<FloatArray2DDenseSIFT> {
  /**
   * Constructor
   * 
   * @param t feature transformation
   */
  public DenseSIFT(final FloatArray2DDenseSIFT t) {
    super(t);
  }

  final static public void addFields(final GenericDialog gd, final FloatArray2DDenseSIFT.Param p) {
  ...
  final static public void readFields(final GenericDialog gd, final FloatArray2DDenseSIFT.Param p) {
  ...
  final public void extractFeatures(final ImageProcessor ip, final Collection<Feature> features) {
    ...
    if (maxSize < ip.getWidth() || maxSize < ip.getHeight()) {
      ...
      final FloatProcessor ipScaled = mpicbg.ij.util.Filter
          .createDownsampled(fp, scale, 0.5f, 0.5f);
    ...
    if (initialSigma < 1.0) {
      ...
      fa = fat;
      initialKernel = Filter.createGaussianKernel(
          (float) Math.sqrt(t.getInitialSigma() * t.getInitialSigma() - 1.0), true);
    } else
      initialKernel = Filter.createGaussianKernel(
          (float) Math.sqrt(initialSigma * initialSigma - 0.25), true);
