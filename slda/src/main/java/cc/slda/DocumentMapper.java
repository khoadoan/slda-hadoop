...
import cc.slda.VariationalInference.ParameterCounter;
...
public class DocumentMapper extends Mapper<IntWritable, Document, PairOfInts, DoubleWritable> {
  ...
  public void map(IntWritable key, Document value,
	...
	//TODO: khoa-should configuration time done only once?
    context.getCounter(ParameterCounter.CONFIG_TIME).increment(configurationTime);
    context.getCounter(ParameterCounter.TOTAL_DOC).increment(1);
    ...
    do {
      ...
      if (Math.random() < 0.01) {
        context.getCounter(ParameterCounter.DUMMY_COUNTER).increment(1);
      }
    ...
    context.getCounter(ParameterCounter.LOG_LIKELIHOOD).increment(
    ...
    if (!learning || !randomStartGamma) {
      //TODO: khoa-check if we need a multi-name output analogy in mapreduce? is baseOuputPath is the equivalent?
