...
public class VariationalInference extends Configured implements Tool, Settings {
	...
	public int run(String[] args) throws Exception {
		...
		options.addOption(OptionBuilder.withArgName(Settings.INTEGER_INDICATOR)
				.hasArg().withDescription("number of terms").isRequired()
				.create(Settings.TERM_OPTION));
		options.addOption(OptionBuilder.withArgName(Settings.INTEGER_INDICATOR)
				.hasArg().withDescription("number of topics").isRequired()
		...
		options.addOption(OptionBuilder
				...
				.withDescription(
						"number of iterations (default - "
								+ Settings.DEFAULT_GLOBAL_MAXIMUM_ITERATION
		...
		options.addOption(OptionBuilder
				.withArgName(Settings.PATH_INDICATOR)
				.hasArgs()
				.withDescription(
						"run program in inference mode, i.e. test held-out likelihood")
				.create(Settings.INFERENCE_MODE_OPTION));
		options.addOption(OptionBuilder.withArgName(Settings.PATH_INDICATOR)
				.hasArgs().withDescription("seed informed prior")
				.create(InformedPrior.INFORMED_PRIOR_OPTION));
		options.addOption(OptionBuilder
				.withArgName(Settings.INTEGER_INDICATOR)
				.hasArg()
				.withDescription(
						"the iteration/index of current model parameters")
		...
		try {
			...
			if (line.hasOption(Settings.HELP_OPTION)) {
				formatter.printHelp(VariationalInference.class.getName(),
						options);
				System.exit(0);
			}
			...
			if (line.hasOption(Settings.ITERATION_OPTION)) {
				if (training) {
					...
					sLogger.info("Warning: " + Settings.ITERATION_OPTION
							+ " ignored in testing mode...");
				}
			}

			if (line.hasOption(Settings.RESUME_OPTION)) {
				snapshotIndex = Integer.parseInt(line
						.getOptionValue(Settings.RESUME_OPTION));
				if (!line.hasOption(Settings.INFERENCE_MODE_OPTION)) {
					resume = true;
					Preconditions
							.checkArgument(snapshotIndex < numberOfIterations);
				}
			}

			if (line.hasOption(Settings.INFERENCE_MODE_OPTION)) {
				if (!line.hasOption(Settings.RESUME_OPTION)) {
					throw new ParseException("Model index missing: "
							+ Settings.RESUME_OPTION
							+ " was not initialized...");
				}
			...
			if (line.hasOption(FileMerger.LOCAL_MERGE_OPTION)) {
				if (training) {
					...
					sLogger.info("Warning: " + FileMerger.LOCAL_MERGE_OPTION
							+ " ignored in testing mode...");
				}
			}
			...
			if (line.hasOption(VariationalInference.MAPPER_COMBINER_OPTION)) {
				if (training) {
					...
					sLogger.info("Warning: "
							+ VariationalInference.MAPPER_COMBINER_OPTION
							+ " ignored in testing mode...");
				}
			}
			...
			if (line.hasOption(VariationalInference.TRUNCATE_BETA_OPTION)) {
				if (training) {
					...
					sLogger.info("Warning: "
							+ VariationalInference.TRUNCATE_BETA_OPTION
							+ " ignored in testing mode...");
				}
			}
			...
			if (line.hasOption(Settings.TOPIC_OPTION)) {
				numberOfTopics = Integer.parseInt(line
						.getOptionValue(Settings.TOPIC_OPTION));
			}

			// TODO: need to relax this contrain in the future
			if (line.hasOption(Settings.TERM_OPTION)) {
				numberOfTerms = Integer.parseInt(line
						.getOptionValue(Settings.TERM_OPTION));
				Preconditions.checkArgument(numberOfTerms > 0,
						"Illegal settings for " + Settings.TERM_OPTION
								+ " option: " + numberOfTerms);
			}
			...
			if (line.hasOption(Settings.RANDOM_START_GAMMA_OPTION)) {
				if (training) {
					...
					sLogger.info("Warning: "
							+ Settings.RANDOM_START_GAMMA_OPTION
							+ " ignored in testing mode...");
				}
			}
			...
			if (line.hasOption(InformedPrior.INFORMED_PRIOR_OPTION)) {
				if (training) {
					informedPrior = new Path(
							line.getOptionValue(InformedPrior.INFORMED_PRIOR_OPTION));
				} else {
					sLogger.info("Warning: "
							+ InformedPrior.INFORMED_PRIOR_OPTION
							+ " ignored in test mode...");
				}
			}

			if (line.hasOption(Settings.MAPPER_OPTION)) {
				mapperTasks = Integer.parseInt(line
						.getOptionValue(Settings.MAPPER_OPTION));
			}

			if (line.hasOption(Settings.REDUCER_OPTION)) {
				if (training) {
					reducerTasks = Integer.parseInt(line
							.getOptionValue(Settings.REDUCER_OPTION));
				} else {
					reducerTasks = 0;
					sLogger.info("Warning: " + Settings.REDUCER_OPTION
							+ " ignored in test mode...");
				}
			}
		...
		return run(inputPath, outputPath, numberOfTopics, numberOfTerms,
				numberOfIterations, mapperTasks, reducerTasks, localMerge,
				training, randomStartGamma, resume, informedPrior, modelPath,
				snapshotIndex, mapperCombiner, truncateBeta);
	}

	private int run(String inputPath, String outputPath, int numberOfTopics,
			int numberOfTerms, int numberOfIterations, int mapperTasks,
			int reducerTasks, boolean localMerge, boolean training,
			boolean randomStartGamma, boolean resume, Path informedPrior,
			String modelPath, int snapshotIndex, boolean mapperCombiner,
		...
		Job job1 = Job.getInstance(getConf(),
				VariationalInference.class.getSimpleName());
		...
		if (informedPrior != null) {
			...
			FileUtil.copy(fs, eta, fs, informedPrior, false,
		...
		Path alphaSufficientStatisticsDir = new Path(tempDir.toString()
				+ Path.SEPARATOR + "part-r-00000");
		String betaGlobDir = tempDir.toString() + Path.SEPARATOR
				+ Settings.BETA + Settings.STAR;
		String phiGlobDir = tempDir.toString() + Path.SEPARATOR + Settings.PHI
				+ Settings.STAR;
		...
		String phiPath = outputPath + Settings.PHI;
		...
		if (!training) {
			...
			if (!resume) {
				...
				try {
					sequenceFileWriter = new SequenceFile.Writer(fs,
							job1.getConfiguration(), alphaDir,
		...
		do {
			job1 = Job.getInstance(getConf(),
					VariationalInference.class.getSimpleName());
			Configuration conf = job1.getConfiguration();

			if (training) {
				job1.setJobName(VariationalInference.class.getSimpleName()
			...
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.informed.prior",
					informedPrior != null);
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.mapper.combiner",
					mapperCombiner);
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.truncate.beta",
					truncateBeta && iterationCount >= 5);

			// conf.setInt("mapred.task.timeout",
			// VariationalInference.DEFAULT_MAPRED_TASK_TIMEOUT);
			// conf.set("mapred.child.java.opts", "-Xmx2048m");

			job1.setNumReduceTasks(reducerTasks);

			if (training) {
				MultipleOutputs.addNamedOutput(job1, Settings.BETA,
						SequenceFileOutputFormat.class, PairOfIntFloat.class,
						HMapIFW.class);
			}

			if (!randomStartGamma || !training) {
				MultipleOutputs.addNamedOutput(job1, Settings.GAMMA,
						SequenceFileOutputFormat.class, IntWritable.class,
						Document.class);
			}

			// Khoa: Add output for Phi
			MultipleOutputs.addNamedOutput(job1, Settings.PHI,
					SequenceFileOutputFormat.class, IntWritable.class,
					HMapIVW.class);
			...
			// TODO: should this be the right way to set compression? Need to
			// verify
			// job1.setCompressMapOutput(false);
			...
			double logLikelihood = -counters.findCounter(
					ParameterCounter.LOG_LIKELIHOOD).getValue()
			...
			double trainingTime = counters.findCounter(
					ParameterCounter.TRAINING_TIME).getValue()
					* 1.0 / numberOfDocuments;
			...
			try {
				...
				sequenceFileReader = new SequenceFile.Reader(fs,
						alphaSufficientStatisticsDir, getConf());
			...
			if (localMerge) {
				betaDir = FileMerger.mergeSequenceFiles(getConf(), betaGlobDir,
						betaPath + (iterationCount + 1), 0,
			...
			// Khoa: merge phi's
			if (localMerge) {
				phiDir = FileMerger.mergeSequenceFiles(getConf(), phiGlobDir,
						phiPath + (iterationCount + 1), 0,
						IntWritable.class, HMapIVW.class, true, true);
			} else {
				phiDir = FileMerger.mergeSequenceFiles(getConf(), phiGlobDir,
						phiPath + (iterationCount + 1), reducerTasks,
						IntWritable.class, HMapIVW.class, true, true);
			}
			...
			if (Math.abs((lastLogLikelihood - logLikelihood)
				...
				sLogger.info("Model converged after " + (iterationCount + 1)
						+ " iterations...");
				break;
			}
		...
		try {
			...
			while (keepGoing) {
				...
				while (true) {
					...
					for (int i = 0; i < numberOfTopics; i++) {
						double stepSize = Math
								.pow(VariationalInference.DEFAULT_ALPHA_UPDATE_DECAY_FACTOR,
										decay)
				...
				for (int j = 0; j < numberOfTopics; j++) {
					...
					if (Math.abs((alphaVectorUpdate[j] - alphaVector[j])
							/ alphaVector[j]) >= VariationalInference.DEFAULT_ALPHA_UPDATE_CONVERGE_THRESHOLD) {
						keepGoing = true;
					}
				}
