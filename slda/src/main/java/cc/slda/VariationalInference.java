...
public class VariationalInference extends Configured implements Tool, Settings {
  ...
  private int run(String inputPath, String outputPath, int numberOfTopics, int numberOfTerms,
    ...
    Job job1 = Job.getInstance(getConf(), VariationalInference.class.getSimpleName());
    ...
    if (informedPrior != null) {
      ...
      FileUtil.copy(fs, eta, fs, informedPrior, false, job1.getConfiguration());
    }
    ...
    if (!training) {
      ...
      if (!resume) {
        ...
        try {
          sequenceFileWriter = new SequenceFile.Writer(fs, job1.getConfiguration(), alphaDir, IntWritable.class,
    ...
    do {
      job1 = Job.getInstance(getConf(), VariationalInference.class.getSimpleName());
      Configuration conf = job1.getConfiguration();
      
      if (training) {
        job1.setJobName(VariationalInference.class.getSimpleName() + " - Iteration "
            + (iterationCount + 1));
      } else {
        job1.setJobName(VariationalInference.class.getSimpleName() + " - Test");
      }
      ...
      if (training) {
        MultipleOutputs.addNamedOutput(job1, Settings.BETA, SequenceFileOutputFormat.class,
            PairOfIntFloat.class, HMapIFW.class);
      }

      if (!randomStartGamma || !training) {
        MultipleOutputs.addNamedOutput(job1, Settings.GAMMA, SequenceFileOutputFormat.class,
      ...
      //TODO: should this be the right way to set compression? Need to verify
      ...
      double logLikelihood = -counters.findCounter(ParameterCounter.LOG_LIKELIHOOD).getValue()
          * 1.0 / Settings.DEFAULT_COUNTER_SCALE;
      sLogger.info("Log likelihood of the model is: " + logLikelihood);

      numberOfDocuments = (int) counters.findCounter(ParameterCounter.TOTAL_DOC).getValue();
      sLogger.info("Total number of documents is: " + numberOfDocuments);
      numberOfTerms = (int) (counters.findCounter(ParameterCounter.TOTAL_TERM).getValue() / numberOfTopics);
      sLogger.info("Total number of term is: " + numberOfTerms);

      double configurationTime = counters.findCounter(ParameterCounter.CONFIG_TIME).getValue()
          * 1.0 / numberOfDocuments;
      sLogger.info("Average time elapsed for mapper configuration (ms): " + configurationTime);
      double trainingTime = counters.findCounter(ParameterCounter.TRAINING_TIME).getValue() * 1.0
          / numberOfDocuments;
